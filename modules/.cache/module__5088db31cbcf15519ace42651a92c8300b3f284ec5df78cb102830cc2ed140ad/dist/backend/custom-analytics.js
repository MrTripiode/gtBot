...
var _default = ({
  ...
  async function update(name, operation, value, racing = false) {
    ...
    const result = await knex('analytics_custom').where('date', today).andWhere('name', name).update('count', operation).then();
  ...
  const countUniqRecords = async (from, to, variable) => {
    ...
    const uniqRecordsQuery = function () {
      this.select(knex.raw('distinct name')).from('analytics_custom').where('date', '>=', from).andWhere('date', '<=', to).andWhere('name', 'LIKE', variable + '~%').as('t1');
    };
  ...
  const getters = {
    count: async function (graph, from, to) {
      ...
      const rows = await knex('analytics_custom').select(['date', knex.raw('sum(count) as count')]).where('date', '>=', from).andWhere('date', '<=', to).andWhere('name', 'LIKE', variable + '~%').groupBy('date').then(rows => {
        return rows.map(row => {
          return { ...row,
    ...
    piechart: async function (graph, from, to) {
      ...
      const rows = await knex('analytics_custom').select(['name', knex.raw('sum(count) as count')]).where('date', '>=', from).andWhere('date', '<=', to).andWhere('name', 'LIKE', variable + '~%').groupBy('name').then(rows => {
        return rows.map(row => {
