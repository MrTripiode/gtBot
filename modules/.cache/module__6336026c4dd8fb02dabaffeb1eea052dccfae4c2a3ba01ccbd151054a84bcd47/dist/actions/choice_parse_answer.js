...
const validateChoice = async data => {
  ...
  if (!choice) {
    choice = _.findKey(data.keywords, keywords => _.some(keywords || [], k => _.includes(event.preview.toLowerCase(), k.toLowerCase()) || event.payload && _.includes(event.payload.text.toLowerCase(), k.toLowerCase())));
  }
